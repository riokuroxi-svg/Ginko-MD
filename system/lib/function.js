...
formatBytes(bytes) {
  ...
  return `${(bytes / 1024 ** i).toFixed(2)} ${[ "B", "KB", "MB", "GB", "TB" ][i]}`;
         ...
         if (progress >= 1) {
            setTimeout(function() {
               ...
               exec(`screenfetch - A Deepin`, (error, stdout, stderr) => {
                  console.log(stdout),
                  console.log(chalk.bgGray("WhatsApp Bot By Ginko"))
               })
            }, 200)
         } else {
      ...
      switch (type) {
         ...
         case "change":
            try {
               ...
               sock.logger.error(`Error al requerir el plugin "${filename(file)}\n${format(e)}"`)
            } finally {
   ...
   timeSpeech() {
      let ucapanWaktu = ""
      let wakt = moment.tz("America/Guayaquil").format("HH:mm")

      if (wakt < "23:59") ucapanWaktu = "Wenass!"
      if (wakt < "18:00") ucapanWaktu = "Buenas noches"
      if (wakt < "17:00") ucapanWaktu = "Buenas tardes"
      if (wakt < "11:00") ucapanWaktu = "Buenas tardes"
      if (wakt < "06:00") ucapanWaktu = "Buen Dia"
      if (wakt < "05:00") ucapanWaktu = "Linda mañana"
      if (wakt < "03:00") ucapanWaktu = "¿Por qué no estás durmiendo??"

      return ucapanWaktu
   }

   timeImage() {
 let waktu = moment.tz("America/Guayaquil").format("HH:mm"), image
