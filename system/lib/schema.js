export default function loadDatabase(m) {
    ...
    if (user) {
      ...
      if (!isNumber(user.limit)) user.limit = 80
    ...
    if (m.isGroup) {
      ...
      if (chat) {
        ...
        if (!isBoolean(chat.antiprivado)) chat.antiprivado = false
        if (!isBoolean(chat.antiinternacional)) chat.antiinternacional = false
        ...
        if (!isBoolean(chat.captcha)) chat.captcha = false
      } else {
      db.chats[m.chat] = {
        antitoxic: false,
        antibot: false,
    ...
    if (typeof sett !== "object") db.settings = {}
      if (sett) {
        ...
        if (!isBoolean(sett.autoReact)) sett.autoReact = false
      } else {
        db.settings = {
        style: 1,
        anticall: true,
