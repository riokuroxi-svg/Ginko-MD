...
async function writeExif(media, metadata) {
    ...
    if (Object.keys(metadata).length != 0) {
        const img = new webp.Image()
        const opt = { 
          packId: metadata?.packId ? metadata.packId : "https://www.instagram.com/__ikg.05", 
          packName: metadata?.packName ? metadata.packName : "Sticker By", 
          packPublish: metadata?.packPublish ? metadata.packPublish : "© Ginko",
          packEmail: metadata?.packEmail ? metadata.packEmail : "__ikg.05", 
          packWebsite: metadata?.packWebsite ? metadata.packWebsite : "https://www.instagram.com/__ikg.05", 
...
async function exifAvatar(buffer, packId, packPubls, categories = [''], extra = {}) {
  ...
  const img = new Image()
  const json = { 
    'sticker-pack-id': 'https://www.instagram.com/__ikg.05', 
