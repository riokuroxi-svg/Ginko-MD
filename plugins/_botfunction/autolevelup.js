...
export async function before(m) {
  ...
  if (!user.autolevelup) return !0
	...
	if (user.level <= 2) {
		...
		user.role = 'Principiante Grade 1 ⚊¹'
	} else if (user.level <= 6) {
		user.role = 'Principiante Grade 2 ⚊²'
	} else if (user.level <= 8) {
		user.role = 'Principiante Grade 3 ⚊³'
	} else if (user.level <= 10) {
		user.role = 'Principiante Grade 4 ⚊⁴'
	} else if (user.level <= 12) {
		user.role = 'intermedial Grade 1 ⚌¹'
	} else if (user.level <= 14) {
		user.role = 'intermedial Grade 2 ⚌²'
	} else if (user.level <= 16) {
		user.role = 'intermedial Grade 3 ⚌³'
	} else if (user.level <= 18) {
		user.role = 'intermedial Grade 4 ⚌⁴'
	} else if (user.level <= 20) {
		user.role = 'intermedial Grade 5 ⚌⁵'
	} else if (user.level <= 22) {
		user.role = 'avanzado Grade 1 ☰¹'
	} else if (user.level <= 24) {
		user.role = 'avanzado Grade 2 ☰²'
	} else if (user.level <= 26) {
		user.role = 'avanzado Grade 3 ☰³'
	} else if (user.level <= 28) {
		user.role = 'avanzado Grade 4 ☰⁴'
	} else if (user.level <= 30) {
		user.role = 'avanzado Grade 5 ☰⁵'
	} else if (user.level <= 32) {
		user.role = 'Sargento Grade 1 ≣¹'
	} else if (user.level <= 34) {
		user.role = 'Sargento Grade 2 ≣²'
	} else if (user.level <= 36) {
		user.role = 'Sargento Grade 3 ≣³'
	} else if (user.level <= 38) {
		user.role = 'Sargento Grade 4 ≣⁴'
	} else if (user.level <= 40) {
		user.role = 'Sargento Grade 5 ≣⁵'
	...
	if (before !== user.level) {
	  let ini_txt = `Felicidades @${m.sender.replace(/@.+/g, '')}, Has subido de nivel!\n\n*-* *Level up* : *${before}* -> *${user.level}*\n*-* *Role up* : *${user.role}*`.trim()
	    try {
	      let image, data, pp
	        try {
